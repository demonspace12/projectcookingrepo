...
public class potrank : MonoBehaviour
{
    public SpriteRenderer potmain2;
    public stagerank timerSC;

    public int potfood_num2 = 0;
    public Shopitem[] pot_2 = new Shopitem[4];
    ...
    public void clickpot2()
    {
        ...
        {
            pot_2[potfood_num2] = timerSC.itemfood[timerSC.foodselect];
            potfood_num2++;
    ...
    {
        Debug.Log("ccc");
        for (int i = 0; i < timerSC.foodpot.Length; i++)
        {
            for (int j = 0; j < pot_2.Length; j++)
            {
                if (pot_2[j].title == timerSC.foodpot[i].Food_material_1 && pot2_material_1 == false)
                {
                    Debug.Log(timerSC.foodpot[i].Food_material_1);
                    pot2_material_1 = true;
                    material_num2 += 1;
                }
                else if (pot_2[j].title == timerSC.foodpot[i].Food_material_2 && pot2_material_2 == false)
                {
                    Debug.Log(timerSC.foodpot[i].Food_material_2);
                    pot2_material_2 = true;
                    material_num2 += 1;
                }
                else if (pot_2[j].title == timerSC.foodpot[i].Food_material_3 && pot2_material_3 == false)
                {
                    Debug.Log(timerSC.foodpot[i].Food_material_3);
                    pot2_material_3 = true;
                    material_num2 += 1;
                }
                else if (pot_2[j].title == timerSC.foodpot[i].Food_material_4 && pot2_material_4 == false)
                {
                    Debug.Log(timerSC.foodpot[i].Food_material_4);
            ...
            if (material_num2 == 4)
            {
                potready2 = true;
                Debug.Log("ready");
                foodcooking2 = timerSC.foodpot[i];
